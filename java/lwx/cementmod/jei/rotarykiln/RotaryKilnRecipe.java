package lwx.cementmod.jei.rotarykiln;
...
public class RotaryKilnRecipe implements IRecipeWrapper {
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ...
        // 显示配方类型标签
        if (isFuelRecipe) {
            String label = TextFormatting.DARK_GRAY + I18n.format("jei.rotary_kiln.fuel_recipe");
            fontRenderer.drawString(label, (recipeWidth - fontRenderer.getStringWidth(label)) / 2, 5, 0x333333);
        } else {
            String label = TextFormatting.DARK_GRAY + I18n.format("jei.rotary_kiln.smelting_recipe");
            fontRenderer.drawString(label, (recipeWidth - fontRenderer.getStringWidth(label)) / 2, 5, 0x333333);
        }
