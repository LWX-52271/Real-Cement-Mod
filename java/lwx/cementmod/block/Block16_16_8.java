...
public class Block16_16_8 extends Block {
    ...
    // 碰撞箱定义 (高度8/16=0.5)
    protected static final AxisAlignedBB PLATE_EMPTY_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);
    protected static final AxisAlignedBB PLATE_SOLID_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);
    protected static final AxisAlignedBB PLATE_WET_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);
    protected static final AxisAlignedBB PLATE_DISMANTLED_AABB = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);

    public Block16_16_8(PlateType type) {
        ...
        String name = "16x16x8_" + type.name().toLowerCase();
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox,
        ...
        switch (type) {
            case EMPTY:
                addCollisionBoxToList(pos, entityBox, collidingBoxes, new AxisAlignedBB(0.0, 0.0, 0.0, 1.0, 0.5, 0.03125));
                addCollisionBoxToList(pos, entityBox, collidingBoxes, new AxisAlignedBB(0.0, 0.0, 0.96875, 1.0, 0.5, 1.0));
                addCollisionBoxToList(pos, entityBox, collidingBoxes, new AxisAlignedBB(0.0, 0.0, 0.0, 0.03125, 0.5, 1.0));
                addCollisionBoxToList(pos, entityBox, collidingBoxes, new AxisAlignedBB(0.96875, 0.0, 0.0, 1.0, 0.5, 1.0));
                break;
            case WET:
            case SOLID:
            case DISMANTLED:
                addCollisionBoxToList(pos, entityBox, collidingBoxes, new AxisAlignedBB(0.0, 0.0, 0.0, 1.0, 0.5, 1.0));
                break;
        }
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player,
        ...
        if (type == PlateType.EMPTY) {
            if (!heldItem.isEmpty() && heldItem.getItem() == ItemRegistryHandler.WET_CONCRETE_BUCKET) {
                int durabilityCost = 8;// 检查耐久度
                if (heldItem.getItemDamage() + durabilityCost <= heldItem.getMaxDamage()) {
                    world.setBlockState(pos, BlockRegistryHandler.BLOCK_16x16x8_WET.getDefaultState());
            ...
            if (!heldItem.isEmpty() && heldItem.getItem().getToolClasses(heldItem).contains("axe")) {
                world.setBlockState(pos, BlockRegistryHandler.BLOCK_16x16x8_DISMANTLED.getDefaultState());
                world.playSound(null, pos, SoundEvents.BLOCK_WOOD_PLACE, SoundCategory.BLOCKS, 0.8F, 1.0F);

                if (!world.isRemote) {
                    spawnAsEntity(world, pos, new ItemStack(Item.getItemFromBlock(BlockRegistryHandler.BLOCK_16x16x8_EMPTY)));
                }
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        if (type == PlateType.WET) {
            ...
            if (!downState.isFullCube() || downBlock.isAir(downState, world, downPos)) {
                ...
                // 将自身变回空框架
                world.setBlockState(pos, BlockRegistryHandler.BLOCK_16x16x8_EMPTY.getDefaultState());
                ...
                if (rand.nextInt(90) < 1) {  // 修改为90秒内硬化
                    world.setBlockState(pos, BlockRegistryHandler.BLOCK_16x16x8_SOLID.getDefaultState());
                    world.playSound(null, pos, SoundEvents.BLOCK_STONE_PLACE, SoundCategory.BLOCKS, 0.8F, 1.0F);
                } else {
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return type == PlateType.SOLID ?
                Item.getItemFromBlock(BlockRegistryHandler.BLOCK_16x16x8_EMPTY) :
